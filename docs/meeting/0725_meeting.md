...
| 9 | '직원명' 필터 추가 | **/reports/monthly** | 신규 | **월간 페이지에 직원명 필터** (인건비 현황이 아님) |

> 이번 회의에서 원안 대비 변경된 항목: **5번(너비 조정 → 차트 삭제)**, **7번(손익표 임베드 → 인건비 카드)**, **9번(/labor → /monthly)**, **6번(상세 설정 필드 구체화)**.
...
## 9. 월간 페이지에 '직원명' 필터 추가 (신규) — **/reports/monthly**

> 원안은 "기간 분석 - 인건비 화면(/reports/labor)"이었으나,本次会议에서 **/reports/monthly 페이지**로 변경.

### 현재 상태
- `/app/reports/monthly` 필터: 조회 월(`src/app/app/reports/monthly/page.tsx:88-99`) / 지점(`100-125`).
- 월간 쿼리는 현재 인건비/직원 데이터를 select하지 않음 (`queries.ts:1988-2219`).

### 확정 방향
- 월간 페이지 필터 영역(조회 월·지점 근처)에 **직원명 필터** 추가.
- 7번(인건비 카드)과 동일하게 월간 쿼리에 인건비/직원 데이터 select가 선행되어야 하므로 **7번과 함께 구현**하는 것이 자연스러움.
- 참고: 인건비 현황(`/reports/labor`)에는 이미 `workerName`이 조회되고 있어(`src/features/labor/headquarters-labor-queries.ts:230-283`) 필터 패턴 재활용 가능.
