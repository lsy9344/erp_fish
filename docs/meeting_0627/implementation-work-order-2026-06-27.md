...
보완 반영: `docs/reference_from_customer/장부-202605현대 (1).xlsx`의 `26` 시트에서 `AE4/AE5`와 `C5/C17`의 위치, 값, 수식이 확인됐다. 이제 `AE4` 의미 확인은 막힌 항목이 아니다. 마진율 반전 표시 요구는 2026-06-30 지시로 삭제됐다.

교차 검토 반영: 6/22~6/27 미팅 폴더 문서와 대조한 상세 부족분, 정책 충돌, 의뢰자 결정 항목은 `docs/meeting_0627/cross-document-gap-review-2026-06-28.md`에 별도로 기록했다. 2026-06-28 의뢰자 답변으로 급여/인건비 노출, Ecount 단가 수정 권한, 전일재고 상세 범위, 알림 채널은 확정됐다.
...
| 냉동/생물 기준 | 자료(`냉동_생물_자료.xlsx`)를 받았다. 최종 용어는 `냉동/생물`로 확정. | 기준표 규칙(`냉)` 접두만 냉동, 나머지 생물)으로 자동 분류 기본값을 채우고 본사가 확정한다. |
...
8. WO-14는 장부 기준과 분석 기준의 매출/이익률 원값을 나란히 보여주고, 두 매출 기준의 차이 금액과 차이율을 본사 전용으로 표시한다. 마진율 반전 표시는 구현 범위에서 제외한다.
...
| 게이트 | 적용 WO | 통과 기준 |
|---|---|---|
| G-01 본사 홈 이중 매출/마진 확인 | WO-14 | 본사 아이디로 `홈` 화면에 접속해 `장부 매출`/`장부 이익률`과 `분석 매출`/`분석 이익률`, 두 매출 기준 차이 금액/차이율이 구분되는지 테스트한다. |
...
| 번호 | 항목 | 상태/기준 | 구현 영향 |
|---:|---|---|---|
| C-01 | 본사 홈 마진율 반전 표시 | 삭제. 2026-06-30 지시로 구현/검수 기준에서 제외한다. | 없음 |
| C-02 | 홈 매출/마진 라벨 | 확정. 가장 쉬운 말로 `장부 매출`, `장부 이익률`, `분석 매출`, `분석 이익률`을 기본값으로 쓴다. | 본사 홈 카드와 리포트 라벨 |
| C-03 | 품목 냉동/생물 기준표 | 확정. 기준표(`냉동_생물_자료.xlsx`)를 받았고 최종 화면 용어는 `냉동/생물`로 확정. 분류 규칙은 `냉)` 접두만 냉동, 나머지 생물. | 품목 master, 이카운트 매핑, 리포트 |
...
7. 냉동/생물 분류는 품목명 문자열 추측으로 확정하지 않는다. 최종 용어는 `냉동/생물`로 확정했고, 기준은 master data 기준표(`냉)` 접두만 냉동)로 정한다.
...
4. 분류가 애매한 신규 품목은 `기준 미정`으로 따로 표시한다(C-03 냉동/생물 기준표 도착 후에도 본사가 수동 확정 전까지).
...
**착수 조건**
C-02는 확정됐다. WO-17의 셀 반영 검증표에서 본사 홈 관련 범위가 정리된 뒤 착수한다. `AE4/AE5`, `C5/C17`의 셀 의미는 확인됐으므로 다시 묻지 않는다. 마진율 반전 표시 요구는 2026-06-30 지시로 삭제됐다.
...
1. `C5/C17` 기준과 `AE4/AE5` 기준을 별도 필드로 모델링한다. 예: `장부 매출`, `장부 이익률`, `분석 매출`, `분석 이익률`.
2. `AE4`는 더 이상 의미 미확정 셀이 아니므로 작업 중 막힌 항목으로 분류하지 않는다.
3. 두 기준의 차이 금액과 차이율을 본사 전용으로 표시한다.
4. 지점장 화면과 지점장 응답에는 차이 금액, 원가, 마진을 보내지 않는다.
5. 기존 이상 신호와 마감 preflight에 영향이 있으면 테스트를 추가한다.
...
- 본사 홈에 두 매출 기준의 차이 금액과 차이율이 본사 전용으로 보인다.
- 지점장에게 민감값이 보이지 않는다.
...
| `기간조회_RAW` | 기준일, 지점, 장부ID, 장부상태, 품목명, 규격, 품목구분, 냉동/생물, 수량, 단위, 매출, 추정매출, 매입원가, 매출이익, 이익률, 손실수량, 손실금액, 재고수량, 장기재고여부, 경과일, 작성자, 수정자 |
| `월별손익` | 기준월, 지점, 매출, 매입원가, 매출이익, 이익률, 인건비, 월세, 관리비, 공과금, 세금/수수료, 포장/소모품, 배송/운반, 수선/유지보수, 기타비용, 본사조정, 남은금액, 조정사유, 메모 |
| `재고현황` | 기준일, 지점, 품목명, 규격, 품목구분, 냉동/생물, 재고수량, 단위, 입고기준일, 경과일, 장기재고기준일, 장기재고여부, 원본이카운트단가, 장부적용단가, 재고금액, 메모 |
| `품목매출` | 조회 시작일, 조회 종료일, 지점, 품목명, 규격, 품목구분, 냉동/생물, 추정판매수량, 추정매출, 추정매입원가, 추정매출이익, 추정이익률, 손실수량, 손실금액, 재고수량 |
...
4. 본사 홈 `AE4/AE5`, `C5/C17` 확인, 전날 재고 보기, xlsx 다운로드, 장기재고 기준일 관리 항목을 추가한다.
...
이 묶음은 홈 표시 라벨이 확정된 전제로 진행한다. `AE4/AE5`, `C5/C17`의 셀 의미는 이미 확인된 전제로 둔다. 마진율 반전 표시는 제외한다.
...
1. 냉동/생물 기준 품목표. (수령 완료: `냉동_생물_자료.xlsx`, 최종 용어 `냉동/생물` 확정)
   - 품목명
   - 표준 품목명
   - 냉동/생물 구분(최종 용어 `냉동/생물`로 확정)
   - 품목 구분
   - 장기재고 기준일
...
- 급여/인건비 노출, 단가 수정 권한, 전일재고 노출 범위, 알림 채널, 사업자명은 확정됐다.
...
- 마진율 반전 표시 요구는 2026-06-30 지시로 삭제됐다.
...
3. 기존 `grossMarginRate` 계산 필드는 덮어쓰지 않는다.
4. 현재 대시보드의 이상 신호가 어떤 마진율을 보는지 명시한다.
5. 지점장 response shaping에 새 필드들이 들어가지 않게 한다.
6. `C5/C17`, `AE4/AE5` 값은 테스트 설명에 근거로 남긴다. 코드 주석에는 필요한 최소 설명만 둔다.
7. WO-17 검증표에서 본사 홈 반영 범위가 정리되기 전에는 홈 이중 매출/마진 구현을 시작하지 않는다.

**취약점**

- `마진율`, `이익률`, `매출 차이`라는 기존 이름에 새 의미를 덮으면 리포트, 이상 신호, 마감 preflight가 동시에 흔들린다.
- 마진율 표시값의 의미를 바꾸면 과거 데이터와 리포트가 달라질 수 있다.

**추가 완료 조건**

- 본사 홈 fixture에서 `C5/C17`, `AE4/AE5` 기준값과 두 매출 기준 차이를 설명한다.
...
2. 검수 항목은 급여/인건비 본사 전용, 단가 본사 전용, 전일재고 민감값 제외, xlsx 다운로드, 장기재고 기준일 관리, 홈 이중 매출/마진 표시를 기준으로 한다.
...
2026-06-28 답변으로 급여/인건비 노출, 단가 수정 권한, 홈 라벨, 전일재고 노출, LINE 채널, 이카운트 자동 등록, 월별 손익/차트 범위, xlsx 다운로드, xlsx 컬럼, 월별 손익 조정 항목, 기존 급여액 처리, 사업자명은 확정됐다. 아래 질문만 남긴다.

| 번호 | 질문 | 필요한 이유 |
|---:|---|---|
| Q-01 (해결) | 냉동/생물 기준 품목표 구성과 최종 용어. → 기준표(`냉동_생물_자료.xlsx`) 수령, 최종 용어 `냉동/생물`로 확정, 분류 규칙 `냉)` 접두만 냉동. | WO-13의 자동 분류와 장기재고 기준 연결에 필요했다. |
