...
| 6.1 장기재고 기준일 메뉴 | `pnpm test:e2e:meeting-0627` (test 5) | PASS | `:163` 장기재고 기준일 저장 |
| 7.1~7.4 변경 이력 한글·링크·작성자 | `pnpm test:unit:file tests/unit/master-data-history.test.mjs` | PASS | 한글 필드명·전후값·사유, 장부 상세 링크, name→email→id |
...
- 5.1~5.3 품목 리포트 제목·원가/마진 본사전용 노출(부분만 E2E로 확인)
- 6.5 장기재고 화면 지점장 vs 본사 노출 항목 차이
- 6.6 알림 LINE 단일 채널
- 8.1 사이드바·메타데이터 표시명(로그인 헤딩은 자동 검증됨)
