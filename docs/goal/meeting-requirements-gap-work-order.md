...
- P1 Task 5 본사 대시보드 자동 갱신 정책을 적용했다. 관제판은 화면이 보이고 서울 시간 05:00 이상 20:00 미만일 때만 30초마다 `router.refresh()`를 실행한다. 화면이 다시 보이거나 영업시간이 시작되면 즉시 한 번 갱신한다. 자동 갱신이 멈춰 있어도 수동 새로고침은 계속 동작한다.
...
- 화면이 보이고 서울 시간 05:00 이상 20:00 미만일 때만 30초 자동 갱신을 실행한다.
- 화면이 숨겨졌거나 영업시간 밖이면 DB 질의를 부르는 자동 갱신을 멈춘다.
- 멈춘 상태에서 다시 보이거나 영업시간이 시작되면 즉시 한 번 갱신하고 주기 갱신을 재개한다.
- 수동 새로고침은 자동 갱신 중지 상태에도 계속 제공한다.
- 갱신 중 로딩 표시와 실패 시 마지막 갱신 시각을 표시한다.
- 상태 변경 후 대시보드에 반영되는 E2E를 추가한다.

**완료 기준**

- 영업시간 중 보이는 화면에서는 사용자가 새로고침하지 않아도 지점 상태 변경이 30초 안에 반영된다.
- 숨김·영업시간 외 상태에서는 자동 갱신이 DB를 깨우지 않는다.
