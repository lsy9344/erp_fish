  ...
  return (
    <div className="flex flex-col gap-4 rounded-md border p-4">
      ...
      <p className="text-muted-foreground text-xs">
        매출/마진은 본사 리포트와 같은 기준(매출원가·정정 반영)으로 계산합니다.
        마진율을 계산할 수 없는 경우 &ldquo;계산 불가&rdquo;와 사유를 함께
        표시합니다.
      </p>
      ...
      {data.unlinkedPayrollRowCount > 0 ? (
        <p className="rounded-md border border-amber-300 bg-amber-50 p-2 text-xs text-amber-800 dark:border-amber-700 dark:bg-amber-950 dark:text-amber-200">
          ⚠ 직원이 연결되지 않은 자유 입력 급여 행이{" "}
          {data.unlinkedPayrollRowCount}건 있습니다. 이 급여는 직원별 분석에
          포함되지 않으니 직원 마스터와 연결해 주세요.
        </p>
      ) : null}
