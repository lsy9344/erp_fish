...
// WO(2026-06-25): 판매 예정가 입력은 3단계 매입 화면으로 통합됐다. 별도 "판매가 계획"
...
test("판매가 계획 메뉴가 기본 지점장 네비게이션에서 보이지 않는다", async ({
  ...
  // 통합 위치인 3단계 매입 화면에는 오늘 팔 가격(예상) 입력 위치가 존재한다.
  await expect(page.getByRole("button", { name: "항목 추가" })).toBeVisible();
});

test("기존 판매가 계획 route는 storeId를 보존한 채 3단계 매입으로 redirect한다", async ({
  page,
}) => {
  await login(page);
