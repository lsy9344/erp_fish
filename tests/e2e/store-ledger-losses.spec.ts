...
test("사유가 비어 있으면 저장을 막고 390px 모바일 입력 상태를 제공한다", async ({
  ...
  await expect(
    page
      .getByText(
        "판매계획가는 3단계 재고에서 입력되며 저장 후 손실 금액에 자동 반영됩니다. 현재는 판매계획가가 없어 손실액이 미산정 상태입니다.",
      )
      .first(),
  ).toBeVisible();
  ...
  const quantityErrorId = quantityDescriptionIds.find((id) =>
    id.endsWith("-error"),
  );
  expect(quantityHelpId).toBeTruthy();
  expect(quantityErrorId).toBeTruthy();
  await expect(page.locator(`[id="${quantityHelpId}"]`)).toContainText(
    "한 박스 100마리 중 10마리를 폐기하면 0.1",
  );
  await expect(page.locator(`[id="${quantityErrorId}"]`)).toContainText(
    "박스단위 수량은 0 이상이고 소수점 첫째 자리까지 입력할 수 있습니다.",
  );
