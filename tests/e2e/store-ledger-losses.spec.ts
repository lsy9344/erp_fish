...
test("손실 항목 여러 건을 저장하고 재방문 시 목록과 합계를 본다", async ({
  ...
  await page.getByLabel("실제 판매/회수액(원)").nth(0).fill("5000");
  ...
  await page.getByLabel("실제 판매/회수액(원)").nth(1).fill("3000");
  ...
  await expect(page.getByLabel("실제 판매/회수액(원)").nth(0)).toHaveValue(
    "5000",
  );
  await expect(page.getByLabel("실제 판매/회수액(원)").nth(1)).toHaveValue(
    "3000",
  );
  await expect(page.getByText("총 손실액")).toHaveCount(0);
...
test("지점장이 손실 유형 표시명(alias)을 바꾸면 처리 유형 선택지에 반영되고, 비우면 본사 등록명으로 되돌아간다", async ({
  ...
  const ledger = await seedTodayLedger(fixtureDate);
  await seedProduct("스토리2-7 alias 광어");
  void ledger;
...
test("사유가 비어 있으면 저장을 막고 390px 모바일 입력 상태를 제공한다", async ({
  ...
  await page.getByLabel("수량").fill("1.25");
  await page.getByLabel("실제 판매/회수액(원)").fill("2000");
  await lossSaveButton(page).click();
  ...
  const amountInput = page.getByLabel("실제 판매/회수액(원)");
  ...
  expect(savedLoss?.quantity.toString()).toBe("1.5");
});
...
test("손실 수량이 재고 흐름보다 크면 저장을 막는다", async ({ page }) => {
  ...
  await page.getByLabel("실제 판매/회수액(원)").fill("27000");
  ...
  const quantityError = page
    .getByRole("alert")
    .filter({
      hasText: `${product.name} / 1kg 손실 수량이 재고보다 많습니다`,
    })
    .first();
...
test("손실 저장 후 재고 기준 수량에 손실 수량을 반영한다", async ({ page }) => {
  ...
  await page.getByLabel("처리 유형").selectOption(lossType.id);
  await page.getByLabel("수량").fill("2");
  await page.getByLabel("실제 판매/회수액(원)").fill("20000");
...
test("손실 라인을 수정하고 삭제하면 version과 감사 로그에 전후 값이 남는다", async ({
  ...
  await page.getByLabel("처리 유형").nth(1).selectOption(discount.id);
  await page.getByLabel("수량").nth(1).fill("1");
  await page.getByLabel("실제 판매/회수액(원)").nth(1).fill("1000");
  ...
  await page.getByLabel("실제 판매/회수액(원)").nth(0).fill("");
  await expect(page.getByLabel("실제 판매/회수액(원)").nth(0)).toHaveValue("");
  await lossSaveButton(page).click();
  await expect(
    page.getByText("실제 판매/회수액은 0원 이상의 정수여야 합니다."),
  ).toBeVisible();
  const amountInput = page.getByLabel("실제 판매/회수액(원)").nth(0);
  ...
  await page.getByLabel("실제 판매/회수액(원)").nth(0).fill("7000");
...
test("stale version 손실 저장은 conflict dialog를 보여주고 입력값을 쓰지 않는다", async ({
  ...
  await page.getByLabel("실제 판매/회수액(원)").fill("3000");
