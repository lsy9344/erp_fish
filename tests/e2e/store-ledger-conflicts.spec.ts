...
test("stale sales 저장은 structured conflict dialog를 보여주고 첫 저장값을 유지한다", async ({
  ...
  await page.goto(`/app/store-entry?storeId=${STORE_ID}&date=${CONFLICT_DATE}`);
...
test("두 브라우저 컨텍스트의 같은 매출 필드 동시 수정은 두 번째 저장을 거부한다", async ({
  ...
  const ledgerPath = `/app/store-entry?storeId=${STORE_ID}&date=${CONFLICT_DATE}`;
  ...
  try {
    ...
    // 작성자 표시명은 1단계 첫 저장 시 필수다(secondPage는 저장 전에 로드되어 편집 가능).
    await secondPage
      .getByRole("textbox", { name: "작성자 표시명", exact: true })
      .fill("현장 박팀장");
    await secondPage
