...
test("재고에서 후속 단계 이동은 discard 선택 없이 저장 성공 뒤에만 진행한다", async ({
  ...
  await expect(
    page
      .getByRole("alert")
      .filter({ hasText: "모든 품목의 판매계획가를 입력해 주세요." })
      .first(),
  ).toBeVisible();
  await expect(
    page.getByRole("dialog", { name: "저장하지 않은 변경이 있습니다" }),
  ).toHaveCount(0);
  await page.getByLabel(`${product.name} 판매계획가`).fill("2000");
  await page.getByRole("link", { name: /4단계: 지출/ }).click();
...
test("화면 밖 수동 추가 품목은 기존 판매계획가를 복원한다", async ({
  page,
}) => {
  const actorId = await getActorId();
  ...
  await expect(page.getByLabel(`${product.name} 판매계획가`)).toHaveValue(
    "16,000",
  );
  await expect(
...
test("FIFO lot 합계가 Int 범위를 넘으면 CAS 전에 행 오류로 차단한다", async ({
  ...
  await page.getByLabel(`${product.name} 판매계획가`).fill("10");
  await page.getByRole("button", { name: "저장", exact: true }).click();
