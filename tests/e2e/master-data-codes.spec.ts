...
async function cleanupStory16Data() {
...
test("본사는 코드 관리 목록과 검색, 그룹, 상태 필터를 볼 수 있다", async ({
  ...
  await expect(page.getByRole("cell", { name: "스토리16 현금" })).toBeVisible();
  await expect(codeRow(page, "스토리16 폐기")).toContainText("비활성");
  ...
  await expect(page.getByRole("cell", { name: "스토리16 현금" })).toBeVisible();
  await expect(page.getByRole("cell", { name: "스토리16 식대" })).toHaveCount(
    0,
  );
  ...
  await expect(page.getByRole("cell", { name: "스토리16 식대" })).toBeVisible();
...
test("본사는 코드가 없어도 세 코드 그룹을 확인하고 선택할 수 있다", async ({
  ...
  await expect(groupList.getByRole("button", { name: "결제수단" })).toBeVisible();
  await expect(groupList.getByRole("button", { name: "비용 항목" })).toBeVisible();
  await expect(groupList.getByRole("button", { name: "손실 유형" })).toBeVisible();
...
test("본사는 코드를 생성, 수정, 비활성 처리하고 감사 로그를 남긴다", async ({
  ...
  const codeName = `스토리16 카드 ${suffix}`;
  const editedName = `스토리16 법인카드 ${suffix}`;
...
test("지점장은 코드 관리 URL에서 데이터를 볼 수 없다", async ({ page }) => {
  ...
  await expect(page.getByText("스토리16 지점장차단")).toHaveCount(0);
});
