...
test("회의 0627 본사 전용 관리와 월간 xlsx 5시트 export를 검증한다", async ({
  ...
  await expect(page.getByRole("row").filter({ hasText: LONG_STOCK_CATEGORY }))
    .toContainText("4일");
  await expect(
    prisma.auditLog.count({ where: { targetType: "LongStockThresholdSetting" } }),
...
async function setEcountUploadFile(
  ...
  const selectedFile = await fileInput.evaluate((input) => {
