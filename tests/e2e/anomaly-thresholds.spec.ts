...
test("본사는 이상 신호 기준값을 저장하고 감사 이력을 남긴다", async ({
  ...
  await expect(page.getByText("마지막 변경")).toBeVisible();
  ...
  await expect
    .poll(async () => {
      ...
      >(
        `SELECT "salesDropRateBps", "grossMarginDropBps", "salesDifferenceAmount", "lossAmount", "inventoryDifferenceQuantity", "isActive"
  ...
  await expect(thresholdHistoryRow.first()).toContainText(
    "정책 확인 전 임시 비활성화",
  );
