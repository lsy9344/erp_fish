...
async function cleanupAnomalyThresholdData() {
  ...
  await prisma.auditLog.deleteMany({
    where: { targetType: "Store", action: "threshold.updated" },
  });
  await prisma.store.updateMany({
    data: { reportMarginGapThresholdBps: 150 },
  });
...
test("본사는 이상 신호 기준값을 저장하고 감사 이력을 남긴다", async ({
  ...
  await page
    .getByRole("list")
    .getByRole("link", { name: "이상 신호", exact: true })
    .click();
...
test("본사는 활성 지점별 리포트 마진 차이 기준을 저장하고 감사 이력을 남긴다", async ({
  page,
}) => {
  const targetStore = await prisma.store.findFirstOrThrow({
    where: { isActive: true },
  ...
  await expect(
    page.getByRole("heading", {
      name: "지점별 리포트 마진 차이 기준",
    }),
  ).toBeVisible();
  const thresholdInput = page.getByLabel(`${targetStore.name} 마진 차이 기준`, {
    exact: true,
  });
  await expect(thresholdInput).toHaveValue("1.5");

  await thresholdInput.fill("2.25");
  await page
    .getByLabel("지점별 기준 변경 사유", { exact: true })
    .fill("지점별 리포트 경보 민감도 조정");
  await page.getByRole("button", { name: "지점별 기준 저장" }).click();

  await expect(
    page.getByText("지점별 마진 차이 기준을 저장했습니다."),
  ).toBeVisible();
  await expect
    .poll(async () => {
      ...
      return store?.reportMarginGapThresholdBps;
    })
    .toBe(225);
  ...
  const auditLogs = await prisma.auditLog.findMany({
    where: {
      targetType: "Store",
      targetId: targetStore.id,
      action: "threshold.updated",
    },
    orderBy: { createdAt: "asc" },
  });
  expect(auditLogs).toHaveLength(1);
  expect(auditLogs[0]?.reason).toBe("지점별 리포트 경보 민감도 조정");
  expect(auditLogs[0]?.before).toMatchObject({
    targetName: targetStore.name,
    reportMarginGapThresholdBps: 150,
  });
  expect(auditLogs[0]?.after).toMatchObject({
    targetName: targetStore.name,
    reportMarginGapThresholdBps: 225,
  });

  await page.reload();
  await expect(
    page.getByLabel(`${targetStore.name} 마진 차이 기준`, { exact: true }),
  ).toHaveValue("2.25");
  await page
    .getByLabel("지점별 기준 변경 사유", { exact: true })
    .fill("값 변경 없는 재저장");
...
test("지점별 리포트 마진 차이 기준은 허용 범위와 변경 사유를 검증한다", async ({
  page,
}) => {
  const targetStore = await prisma.store.findFirstOrThrow({
    where: { isActive: true },
...
test("본사는 장기재고 기준일을 품목군별로 저장하고 감사 이력을 남긴다", async ({
  ...
  await login(page, "hq@example.com");
  const longStockThresholdLink = page
    .getByRole("list")
    .getByRole("link", { name: "장기재고 기준일", exact: true });
  await expect(longStockThresholdLink).toHaveAttribute(
    "href",
