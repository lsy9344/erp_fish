...
test("매입 화면은 판매계획가를 다루지 않고 기존 계획을 보존한다", async ({
  ...
  await login(page);
  const actorId = await getManagerUserId();
  const { product } = await seedPurchaseOption("스토리2-4 계획보존", 12000);
  ...
  await page.goto(`/app/store-entry?storeId=${STORY_STORE_ID}&step=purchase`);
  await expect(page.getByLabel("오늘 팔 가격(예상)")).toHaveCount(0);
  ...
  const plan = await prisma.storeSalesPricePlan.findUnique({
    where: {
      storeSalesPricePlan_storeId_businessDate_productId_key: {
        storeId: STORY_STORE_ID,
        businessDate: new Date(`${SELECTED_LEDGER_DATE}T00:00:00.000Z`),
        productId: product.id,
      },
  ...
  expect(plan).toEqual({ plannedUnitPrice: 15000 });
});
