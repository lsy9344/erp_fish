...
test("지점장은 매입 항목을 여러 건 저장하고 재방문 시 목록과 합계를 본다", async ({
  ...
  await page.getByLabel("수량").nth(0).fill("2.5");
  ...
  ).toContainText("48,000원");

  const savedFirstPurchase = await prisma.ledgerPurchaseItem.findFirst({
    where: { productId: first.product.id },
  });
  expect(savedFirstPurchase?.quantity.toString()).toBe("2.5");
...
test("같은 품목의 오늘 팔 가격은 모든 행에서 동기화되고 하나의 계획으로 저장된다", async ({
  page,
}) => {
  await login(page);
  ...
  const plans = await prisma.storeSalesPricePlan.findMany({
    where: {
      storeId: STORY_STORE_ID,
      businessDate: new Date(`${SELECTED_LEDGER_DATE}T00:00:00.000Z`),
      productId: product.id,
  ...
  expect(plans).toEqual([{ plannedUnitPrice: 16000 }]);
});
  ...
  await expect
    .poll(() =>
      prisma.ledgerPurchaseItem.findFirst({
        ...
        select: { productName: true, productSpec: true },
      }),
    )
    .toEqual({ productName: product.name, productSpec: product.spec });
});
...
test("매입 단계는 음수와 소수점 둘째 자리 입력을 다른 값으로 바꾸지 않고 검증한다", async ({
  page,
}) => {
  await login(page);
