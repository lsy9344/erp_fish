...
test("전일 근거가 없으면 근거 없는 활성 품목을 자동 표시하지 않고 품목 추가로만 입력한다", async ({
  ...
  await page
    .getByLabel("추가할 품목 선택")
    .selectOption({ label: `${product.name} / ${product.spec}` });
...
test("매입 품목은 당일재고를 빈칸으로 시작하고 미입력이면 저장을 막는다", async ({
  ...
  // 매입 6, 남은 2 = 4개 판매. 정상 판매 소진이라 조정 사유 없이 한 번에 저장된다.
  // (손실은 재고보다 앞 단계라 재고 저장 시점에 이미 입력돼 있으므로 별도 확인 없음.)
...
test("월초 스냅샷에 누락된 활성 품목은 자동 표시하지 않고 품목 추가로만 입력한다", async ({
  ...
  await page
    .getByLabel("추가할 품목 선택")
    .selectOption({ label: `${missingProduct.name} / ${missingProduct.spec}` });
  await page.getByRole("button", { name: "추가" }).click();
  await expect(
