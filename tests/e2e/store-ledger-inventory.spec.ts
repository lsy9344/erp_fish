...
async function fillVisiblePlannedUnitPrices(page: Page) {
  ...
  if (selectedTabIndex >= 0) {
    await tabs.nth(selectedTabIndex).click();
  }
}
...
async function cleanupStoryTwoFiveData() {
  ...
  if (ledgerIds.length > 0) {
    await prisma.ledgerInventoryConversion.deleteMany({
...
test("가로 재고 카드는 데스크톱 두 품목과 모바일 한 품목을 순서대로 배치한다", async ({
    ...
    inventoryItems.push(
      await prisma.ledgerInventoryItem.create({
        data: {
          ...
          previousQuantity: index === 0 ? 0 : 2,
          purchasedQuantity: index === 0 ? 8 : 0,
          currentQuantity: index === 0 ? 8 : 2,
          quantity: index === 0 ? 8 : 2,
          inventoryAmount: index === 0 ? 92_000 : 24_000,
          isModified: false,
          createdById: actorId,
          updatedById: actorId,
        },
  ...
  const oldPurchaseOrigin = `layout-source-old-${product.id}`;
  const newPurchaseOrigin = `layout-source-new-${product.id}`;
  await prisma.ledgerPurchaseItem.createMany({
    data: [
      {
        id: oldPurchaseId,
        lotOriginKey: oldPurchaseOrigin,
        dailyLedgerId: ledger.id,
        productId: product.id,
        sourceType: "MANUAL",
        productName: product.name,
    ...
    data: [
      {
        lotOriginKey: oldPurchaseOrigin,
        dailyLedgerId: ledger.id,
        ledgerInventoryItemId: inventoryItems[0]!.id,
        productId: product.id,
        sourceType: "PURCHASE",
        sourcePurchaseItemId: oldPurchaseId,
        unitPrice: 10_000,
        originalQuantity: 2,
        consumedQuantity: 0,
        remainingQuantity: 2,
      ...
      },
      {
        ...
        productId: product.id,
        sourceType: "PURCHASE",
        sourcePurchaseItemId: newPurchaseId,
        unitPrice: 12_000,
        originalQuantity: 6,
        consumedQuantity: 0,
        remainingQuantity: 6,
        originalAmount: 72_000,
        consumedAmount: 0,
        remainingAmount: 72_000,
        sortOrder: 1,
        sourceBusinessDate: ledger.closingDate,
      },
      {
        lotOriginKey: `layout-target-${secondProduct.id}`,
        dailyLedgerId: ledger.id,
        ledgerInventoryItemId: inventoryItems[1]!.id,
  ...
  expect(desktopFirstBox?.height).toBeCloseTo(desktopSecondBox?.height ?? 0, 0);
  const firstActions = firstRow.locator("[data-inventory-card-actions]");
  const secondActions = secondRow.locator("[data-inventory-card-actions]");
  const firstActionsBox = await firstActions.boundingBox();
  const secondActionsBox = await secondActions.boundingBox();
  expect(
    Math.abs((firstActionsBox?.y ?? 0) - (secondActionsBox?.y ?? 0)),
  ...
  const convertedSource = convertedItems.find(
  ...
  expect(Number(saleAndLossPollution._sum.soldQuantity)).toBe(0);
  expect(Number(saleAndLossPollution._sum.lossQuantity)).toBe(0);
  await expect
    .poll(() =>
...
test("월초 스냅샷 기준 전일재고를 프리필하고 저장 후 수정 행을 유지한다", async ({
  ...
  const savedProductRow = await prisma.ledgerInventoryItem.findUnique({
    where: {
      dailyLedgerId_productId: {
        dailyLedgerId: ledger.id,
        productId: product.id,
...
test("지점장은 전날 재고를 분류하고 판매량·마지막 입고일만 안전하게 본다", async ({
  ...
  await page.goto(`/app/store-entry/inventory?storeId=${STORY_STORE_ID}`);
  const previousStockButton = page.getByRole("button", {
    name: "전날 재고 보기",
    exact: true,
  });
  await expect(previousStockButton).toHaveCount(1);
  await previousStockButton.click();
  ...
  await previousStockButton.focus();
  await expect(previousStockButton).toBeFocused();
...
test("직전 장부가 미마감이면 검토 필요 후보를 보여주고 저장을 허용한다", async ({
  ...
  test.skip(
    getCurrentKstDayOfMonth() === 1,
    "월 1일에는 같은 달의 이전 영업일 미마감 장부가 존재할 수 없어 월초 이월 흐름으로 검증한다.",
  );
...
test("전일 근거가 없으면 근거 없는 활성 품목을 자동 표시하지 않고 품목 추가로만 입력한다", async ({
  ...
  await page.getByLabel(`${product.name} 매입단가`).fill("7000");
  await fillVisiblePlannedUnitPrices(page);
  await page.getByRole("button", { name: "저장", exact: true }).click();
  await expectInventorySaveSucceeded(page);
  ...
  expect(savedItem?.unitPrice).toBe(7000);
  expect(savedItem?.quantity?.toString()).toBe("3");
  expect(savedItem?.inventoryAmount).toBe(21000);

  const fifoLot = await prisma.ledgerInventoryFifoLot.findFirst({
  ...
  expect(fifoLot?.unitPrice).toBe(7000);
  expect(fifoLot?.originalQuantity.toString()).toBe("3");
  expect(fifoLot?.originalAmount).toBe(21000);
  expect(fifoLot?.remainingQuantity.toString()).toBe("3");
  expect(fifoLot?.remainingAmount).toBe(21000);
    ...
    data: [
      {
        ...
        productId: todayProduct.id,
        productName: todayProduct.name,
        productCategory: todayProduct.category,
        productSpec: todayProduct.spec,
        unitPrice: 10_000,
        quantity: 2,
        amount: 20_000,
        createdById: actorId,
...
test("매입 품목은 당일재고를 빈칸으로 시작하고 손실 검토 전 재고 저장을 막는다", async ({
  ...
  const row = page.locator("tr").filter({ hasText: product.name });
  const quantityError = row.getByRole("alert").filter({
    hasText:
  ...
  await expect(quantityError).not.toBeVisible();
  await expect(currentQuantityInput).not.toHaveAttribute(
    "aria-invalid",
    "true",
  );
  ...
  await expect(
    page
      .getByRole("alert")
      .filter({ hasText: "2단계 손실/폐기 단계를 먼저 저장해 주세요." }),
  ).toBeVisible();
  expect(
    await prisma.ledgerInventoryItem.count({
      where: { dailyLedgerId: ledger.id, productId: product.id },
  ...
  // 손실이 없더라도 2단계를 저장하면 "검토 완료"로 기록되고 재고 저장을 진행할 수 있다.
...
test("매입 품목 당일재고 미입력 상태로 진입 직후 이동해도 미저장 경고가 뜨지 않는다", async ({
  ...
  await page.getByRole("link", { name: "1단계: 매입" }).click();
...
test("오늘이 아닌 재고 입력 URL은 지점장 접근을 차단한다", async ({ page }) => {
  ...
  await page.goto(
    `/app/store-entry/inventory?storeId=${STORY_STORE_ID}&date=${getPreviousKstMidnight().toISOString().slice(0, 10)}`,
  );
...
test("stale version 재고 저장은 conflict dialog를 보여주고 입력값을 쓰지 않는다", async ({
  ...
  await currentQuantityInput.fill("4");
  await reasonInput.fill("충돌 뒤에도 남을 초안");
  await fillVisiblePlannedUnitPrices(page);
  await page.getByRole("button", { name: "저장", exact: true }).click();
  ...
  await expect(currentQuantityInput).toHaveValue("4");
  await expect(reasonInput).toHaveValue("충돌 뒤에도 남을 초안");
...
test("전일·매입·손실·당일재고가 모두 0인 품목은 목록에서 숨기고 품목 추가로 복원한다", async ({
  ...
  // 0으로 저장하면 성공 응답에서도 다시 숨겨지고, 품목 추가 후보로는 남는다.
  await fillVisiblePlannedUnitPrices(page);
  await page.getByRole("button", { name: "저장", exact: true }).click();
  await expectInventorySaveSucceeded(page);
  await expect(
