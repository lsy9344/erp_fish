...
async function fillVisiblePlannedUnitPrices(page: Page) {
  ...
  for (let tabIndex = 0; tabIndex < tabCount; tabIndex += 1) {
    ...
    const inputs = page.locator(
      'input[aria-label$=" 판매한 가격"]:visible, input[aria-label$=" 판매가"]:visible',
    );
    for (
      let inputIndex = 0;
...
test("실제 재고 차이를 바꾼 이유와 함께 저장하고 재방문 시 표시한다", async ({
  ...
  expect(adjustment?.beforeQuantity.toString()).toBe("7");
  expect(adjustment?.afterQuantity.toString()).toBe("9");
  expect(adjustment?.differenceQuantity.toString()).toBe("2");
...
test("손실 저장 후 기존 재고 조정의 기준 수량과 차이를 재계산한다", async ({
  ...
  expect(adjustment?.beforeQuantity.toString()).toBe("10");
  expect(adjustment?.afterQuantity.toString()).toBe("12");
  expect(adjustment?.differenceQuantity.toString()).toBe("2");
  ...
  await page.getByLabel("떨이로 실제 판매한 금액").fill("33000");
  ...
  expect(adjustment?.beforeQuantity.toString()).toBe("7");
  expect(adjustment?.afterQuantity.toString()).toBe("12");
  expect(adjustment?.differenceQuantity.toString()).toBe("5");
});
