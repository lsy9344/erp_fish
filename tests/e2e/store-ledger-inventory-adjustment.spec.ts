...
test("실제 재고 차이를 바꾼 이유와 함께 저장하고 재방문 시 표시한다", async ({
  ...
  await expect(page.getByLabel(`${product.name} 바꾼 이유`)).toHaveValue(
    "실사 재고 차이",
  );
...
test("바꾼 이유가 비어 있으면 저장을 막고 이유 필드에 포커스한다", async ({
  ...
  const reasonInput = page.getByLabel(`${product.name} 바꾼 이유`);
...
test("검토 대기 장부에서도 권한 있는 사용자가 원본 재고 조정을 저장한다", async ({
  ...
  await page.getByLabel(`${product.name} 바꾼 이유`).fill("검토 중 실사 차이");
  await getAdjustmentButton(page, product.name).click();
...
test("손실 저장 후 기존 재고 조정의 기준 수량과 차이를 재계산한다", async ({
  ...
  await page.getByLabel(`${product.name} 바꾼 이유`).fill("손실 반영 전 실사");
  await getAdjustmentButton(page, product.name).click();
  await expectAdjustmentSaveSucceeded(page);
