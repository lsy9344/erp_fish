...
test("비로그인 사용자가 루트 경로를 열면 로그인으로 안내된다", async ({
  page,
}) => {
  await page.goto("/");
...
test("비로그인 사용자가 본사 대시보드 URL을 직접 열어도 로그인으로 안내된다", async ({
  page,
}) => {
  ...
  await expect(page.getByText("활성 지점")).toHaveCount(0);
  await expect(page.getByText("기준정보 데이터")).toHaveCount(0);
});
...
test("키보드 제출로 잘못된 로그인을 시도해도 오류가 비밀번호 필드와 연결된다", async ({
  page,
}) => {
  await page.goto("/login");
...
test("본사 계정으로 로그인하면 본사 업무 셸과 사이드바가 보인다", async ({
  ...
  for (const item of ["홈", "리포트", "기준정보"]) {
    await expect(page.getByRole("link", { name: item })).toBeVisible();
  }
  await expect(page.getByRole("link", { name: "설정" })).toHaveCount(0);
});

test("로그인 callbackUrl은 안전한 본사 업무 경로로 사용자를 돌려보낸다", async ({
  page,
}) => {
  await page.goto("/login?callbackUrl=%2Fapp%2Fdashboard");
...
test("로그인 사용자가 루트 경로를 열면 역할별 업무 홈으로 이동한다", async ({
  page,
}) => {
  await page.goto("/login");
...
test("지점장 계정은 본사 대시보드에 직접 접근해도 서버 보호에 막힌다", async ({
  page,
}) => {
  await page.goto("/login");
...
test("로그인 화면은 라이트와 다크 토큰에서 기본 텍스트와 버튼 대비를 유지한다", async ({
  page,
}) => {
  await page.goto("/login");
  ...
  await expect(loginButton).toHaveCSS("background-color", "rgb(96, 165, 250)");
  await expect(loginButton).toHaveCSS("color", "rgb(10, 15, 30)");
});
...
test("활성 배정이 없는 지점장은 본사 문의 안내로 이동한다", async ({
  ...
  await page.getByLabel("로그인 식별자").fill("unassigned-manager@example.com");
  await page.getByLabel("비밀번호").fill("correct-password");
  await page.getByRole("button", { name: "로그인" }).click();
...
test("390px 모바일 지점장 화면은 하단 업무 탭만 터치 가능하게 보여준다", async ({
  ...
  const workspaceNav = page.getByLabel("지점장 하단 업무");
...
test("지점장 업무 진입점은 장부 저장 기능 없이 준비 화면으로 이동한다", async ({
  ...
  await expect(page.getByText(/강남점 · 영업일:/)).toBeVisible();
  await expect(
    page.getByRole("link", { name: /2단계: 손실\/폐기/ }),
...
test("비활성 지점만 배정된 지점장은 본사 문의 안내만 본다", async ({
  ...
  await page.getByLabel("로그인 식별자").fill("inactive-manager@example.com");
  await page.getByLabel("비밀번호").fill("correct-password");
  await page.getByRole("button", { name: "로그인" }).click();
