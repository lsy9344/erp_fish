...
test("지점장은 오늘 장부에서 매출/결제 단계를 본다", async ({ page }) => {
  ...
  // 매출/결제는 6단계로 이동했으므로 명시적으로 해당 단계를 연다(기본 진입은 1단계 매입).
...
test("지점장은 선택 날짜 장부를 열고 재방문 시 같은 장부를 본다", async ({
  ...
  await expect(page.getByLabel("영업일")).toHaveValue(SELECTED_LEDGER_DATE);
  await expect(
    page.getByRole("link", { name: /6단계: 매출\/결제.*저장됨/ }),
...
test("작성자 표시명은 저장 후 2~7단계와 재방문에서도 유지되고 audit actor와 분리된다", async ({
  ...
  await loginAsStoreManager(page);
  // 작성자 표시명은 1단계 매입(기본 진입 단계)에서 입력·저장한다.
  await page.goto(
  ...
  // 작성자 표시명은 1단계 매입 저장(ledger.purchases.saved)에서 기록된다.
...
test("지점장은 매출/결제 금액을 저장하고 재방문 시 유지된다", async ({
  ...
  const expenseInputCode = await prisma.ledgerInputCode.create({
    data: {
      group: "EXPENSE_ITEM",
  ...
  await page.goto("/app/store-entry?storeId=store-gangnam&step=cost");
  await page.getByRole("button", { name: "항목 추가" }).click();
  const expenseCode = page.getByLabel("지출 항목", { exact: true });
  await expenseCode.selectOption(expenseInputCode.id);
  await page.getByRole("textbox", { name: "지출 금액" }).fill("4000");
  await page
  ...
  const savedExpense = await prisma.ledgerExpense.findFirst({
    where: {
      dailyLedger: {
        storeId: STORE_ID,
        closingDate: new Date(`${SELECTED_LEDGER_DATE}T00:00:00.000Z`),
  ...
  expect(savedExpense?.amount).toBe(4000);

  await page.goto("/app/store-entry?storeId=store-gangnam&step=sales");

  const total = page.getByRole("textbox", { name: "총매출", exact: true });
  const carryover = page.getByRole("textbox", {
    name: "이월 매출",
    exact: true,
  });
  ...
  const savedSales = await prisma.dailyLedger.findUniqueOrThrow({
    where: {
      storeId_closingDate: {
        storeId: STORE_ID,
        closingDate: new Date(`${SELECTED_LEDGER_DATE}T00:00:00.000Z`),
    ...
    select: { totalSalesAmount: true, carryoverSalesAmount: true },
  });
  expect(savedSales).toEqual({
    totalSalesAmount: 10_000,
    carryoverSalesAmount: 2_500,
  });
...
test("저장 실패 시 한국어 오류와 재시도 동작이 표시된다", async ({ page }) => {
  await loginAsStoreManager(page);
  // 매출/결제는 6단계로 이동했으므로 라우트 차단 전에 해당 단계를 연다.
  await page.goto("/app/store-entry?storeId=store-gangnam&step=sales");
