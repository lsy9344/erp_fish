...
test("지점장은 오늘 장부에서 매출/결제 단계를 본다", async ({ page }) => {
  ...
  await expect(
    page.locator("li").filter({ hasText: "1단계: 매출/결제" }),
  ).toHaveAttribute(
    "aria-current",
    "step",
  );
  ...
  await expect(page.getByRole("textbox", { name: "카드", exact: true })).toBeVisible();
  await expect(
    page.getByRole("textbox", { name: "기타 결제수단", exact: true }),
  ).toBeVisible();
});

test("지점장은 매출/결제 금액을 저장하고 재방문 시 유지된다", async ({ page }) => {
  await loginAsStoreManager(page);
  ...
  const other = page.getByRole("textbox", { name: "기타 결제수단", exact: true });
  ...
  await page.reload();
  await expect(total).toHaveValue("10000");
  await expect(cash).toHaveValue("3000");
  await expect(card).toHaveValue("2000");
  await expect(other).toHaveValue("1000");
  await expect(page.getByText(/결제 합계 차액.*4,000원/)).toBeVisible();
  await expect(save).toBeVisible();
});

test("지점장은 권한 없는 지점 URL로 이동하면 권한 없음 화면만 본다", async ({ page }) => {
  await loginAsStoreManager(page);
...
test("저장 실패 시 한국어 오류와 재시도 동작이 표시된다", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "기타 결제수단", exact: true }).fill("500");
