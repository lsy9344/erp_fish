...
test("본사 관제판 표시 밀도를 변경하면 요약 카드/표 레이아웃이 바뀌고 URL에 유지된다", async ({
  ...
  await page
    ...
    .click();
  await expect(page).toHaveURL(/density=wide/);
...
test("기준값이 저장된 관제판은 정책 확인 필요 신호와 상세 이동을 제공한다", async ({
  page,
}) => {
  ...
  const reviewRow = getDesktopRow(page, STORE_IDS.review);
  await expect(reviewRow).toContainText("확인 필요");
  await expect(reviewRow).toContainText("재고 기준 확인");
  ...
  await expect(
    reviewRow.getByLabel(/재고 기준 확인: OQ-7\/OQ-17/),
  ).toBeVisible();
  ...
  await expect(page.getByText("재고 기준 확인")).toBeVisible();
  await expect(page.getByText(/OQ-7\/OQ-17/)).toBeVisible();
...
test("390px 모바일 관제판은 기준 확인 필요 복수 chip을 줄바꿈해 표시한다", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await seedStoryThreeThreeThresholds();
  ...
  await expect(signal).toContainText("재고 기준 확인");
