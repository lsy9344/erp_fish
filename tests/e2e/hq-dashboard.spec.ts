...
async function expectNoOverlap(first: Locator, second: Locator) {
  const firstBox = await getVisibleBoundingBox(first);
  const secondBox = await getVisibleBoundingBox(second);
...
async function navigateByLinkHref(
  page: Page,
  link: Locator,
  expectedUrl: RegExp,
) {
...
test("본사 관제판은 활성 지점 전체와 장부 상태를 보여준다", async ({
  ...
  await expect(getDesktopRow(page, STORE_IDS.review)).toContainText(
    "검토 대기",
  );
...
test("본사 관제판 표시 밀도를 변경하면 요약 카드/표 레이아웃이 바뀌고 URL에 유지된다", async ({
  ...
  // 기본 밀도.
  await expect(summary).toHaveAttribute("data-density", "default");
  await expect(tableContainer).toHaveAttribute("data-density", "default");
  ...
  await navigateByLinkHref(
    page,
    page
      .getByLabel("관제판 표시 밀도 선택")
      .getByRole("link", { name: "넓게" }),
    /density=wide/,
  );
  ...
  await expect(
    page.getByTestId("hq-dashboard-column-header-store"),
  ).toBeVisible();
});
...
test("본사 관제판 데스크톱 컬럼 폭은 저장되고 초기화할 수 있다", async ({
  ...
  await storeResizer.focus();
  await storeResizer.press("ArrowRight");
  await storeResizer.press("ArrowRight");
  await storeResizer.press("ArrowRight");
...
test("본사 관제판은 오늘/어제 전환과 필터 빈 상태를 구분한다", async ({
  ...
  await navigateByLinkHref(
    page,
    page.getByLabel("조회 날짜 선택").getByRole("link", { name: "어제" }),
    /\/app\/dashboard\?date=yesterday/,
  );
  await expect(getDesktopRow(page, STORE_IDS.empty)).toContainText("미입력");
...
test("조회 전용 본사는 관제판과 상세에서 mutation action을 볼 수 없다", async ({
  ...
  await expect(
    page.getByRole("button", { name: /본사 마감|정정 기록 저장/ }),
  ).toHaveCount(0);
  await expect(
  ...
  await expect(page.getByRole("region", { name: "정정 기록" })).toHaveCount(0);
  await expect(page.getByLabel("정정 대상")).toHaveCount(0);
  await expect(page.getByLabel("정정값")).toHaveCount(0);
  await expect(page.getByLabel("정정 사유")).toHaveCount(0);
  await expect(page.locator('a[href$="#correction-timeline"]')).toHaveCount(0);
});
...
test("기준값이 저장된 관제판은 재고 이상 신호와 상세 이동을 제공한다", async ({
  ...
  await expect(reviewRow).not.toContainText("매출 기준 확인");
  await expect(reviewRow).not.toContainText("이익률 기준 확인");
  await expect(reviewRow).not.toContainText("매출차액 기준 확인");
  await expect(reviewRow).not.toContainText("손실 기준 확인");
  await expect(reviewRow).not.toContainText("매출차액 초과");
  ...
  await expect(page.getByText(/스토리3-1 테스트 품목 재고 차이/)).toBeVisible();
  await expect(page.getByText("매출 기준 확인")).toHaveCount(0);
  await expect(page.getByText("이익률 기준 확인")).toHaveCount(0);
  await expect(page.getByText("매출차액 기준 확인")).toHaveCount(0);
  await expect(page.getByText("손실 기준 확인")).toHaveCount(0);
...
test("정정 저장 후 관제판 기본 매출은 정정 반영값으로 갱신된다", async ({
  ...
  await expect(closedRow).not.toContainText("매출 기준 확인");
});
...
test("지점장은 마감 장부의 본사 정정 이력을 읽기 전용으로 본다", async ({
  ...
  const storeEntryHtml = await page.content();
  expect(storeEntryHtml).not.toContain("HqDashboardRow");
  expect(storeEntryHtml).not.toContain("기준값");
  expect(storeEntryHtml).not.toContain("스토리3-1 검토대기점");
});

test("지점장 검토 단계는 본사 관제판 shape와 민감 지표를 내려받지 않는다", async ({
  page,
}) => {
  await seedStoryThreeThreeThresholds();
  await login(page, "manager@example.com");
  await page.goto(`/app/store-entry?storeId=${STORE_IDS.closed}&step=review`);

  const metrics = page.locator("section").filter({ hasText: "검토 요약" });

  await expect(metrics).toBeVisible();
  await expect(metrics).toContainText("총매출");
  // 미팅 결정(2026-06-21): 마진율(%)과 총 재고금액은 지점장에게 노출한다.
  await expect(metrics).toContainText("마진율");
  await expect(metrics).toContainText("재고금액");
  // 매출차액, 기준값(임계값), 관제판 shape, 내부 필드 키는 계속 차단한다.
  await expect(metrics).not.toContainText("매출차액");
  await expect(metrics).not.toContainText("기준값");

  const storeEntryText = await page.locator("main").innerText();
  expect(storeEntryText).not.toContain("HqDashboardRow");
  expect(storeEntryText).not.toContain("grossMarginRate");
  expect(storeEntryText).not.toContain("inventoryAmount");
  expect(storeEntryText).not.toContain("salesDifference");
  expect(storeEntryText).not.toContain("threshold");
  expect(storeEntryText).not.toContain("스토리3-1 검토대기점");
});
...
test("390px 모바일 관제판은 핵심 상태가 겹치지 않고 보인다", async ({
  ...
  await expect(status).toContainText("검토 대기");
...
test("390px 모바일 관제판은 재고 이상 chip을 줄바꿈해 표시한다", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await seedStoryThreeThreeThresholds();
  ...
  await expect(row).toBeVisible();
  await expect(row).toContainText("재고 이상");
  await expect(signal).toBeVisible();
  await expect(signal).toContainText("재고 이상");
  await expect(signal).not.toContainText("매출 기준 확인");
  await expect(signal).not.toContainText("이익률 기준 확인");
  await expect(signal).not.toContainText("매출차액 기준 확인");
  await expect(signal).not.toContainText("손실 기준 확인");

  const rowBox = await getVisibleBoundingBox(row);
  const signalBox = await getVisibleBoundingBox(signal);
  expect(signalBox.x).toBeGreaterThanOrEqual(rowBox.x);
