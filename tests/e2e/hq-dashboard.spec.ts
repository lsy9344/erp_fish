...
test("조회 전용 본사는 관제판과 상세에서 mutation action을 볼 수 없다", async ({
  ...
  await expect(
    page.locator('a[href$="#correction-timeline"]'),
  ).toHaveCount(0);
});
...
test("기준값이 저장된 관제판은 정책 확인 필요 신호와 상세 이동을 제공한다", async ({
  ...
  await expect(reviewRow).toContainText("손실 기준 확인");
  ...
  await expect(reviewRow.getByLabel(/매출차액 기준 확인: OQ-1/)).toBeVisible();
  await expect(
    reviewRow.getByLabel(/재고 기준 확인: OQ-7\/OQ-17/),
  ).toBeVisible();
  await expect(reviewRow.getByLabel(/손실 기준 확인: OQ-9/)).toBeVisible();
  ...
  await expect(page.getByText("손실 기준 확인")).toBeVisible();
  await expect(page.getByText(/OQ-9/)).toBeVisible();
...
test("지점장 검토 단계는 본사 관제판 shape와 민감 지표를 내려받지 않는다", async ({
  ...
  const storeEntryHtml = await page.content();
  expect(storeEntryHtml).not.toContain("HqDashboardRow");
  expect(storeEntryHtml).not.toContain("grossMarginRate");
  expect(storeEntryHtml).not.toContain("inventoryAmount");
  expect(storeEntryHtml).not.toContain("salesDifference");
  expect(storeEntryHtml).not.toContain("threshold");
  expect(storeEntryHtml).not.toContain("스토리3-1 검토대기점");
});
...
test("390px 모바일 관제판은 기준 확인 필요 복수 chip을 줄바꿈해 표시한다", async ({
  ...
  await expect(signal).toContainText("매출 기준 확인");
  await expect(signal).toContainText("이익률 기준 확인");
  await expect(signal).toContainText("매출차액 기준 확인");
  await expect(signal).toContainText("재고 기준 확인");
  await expect(signal).toContainText("손실 기준 확인");
  await expect(signal.getByLabel(/매출차액 기준 확인: OQ-1/)).toBeVisible();
