...
test("검토 화면은 지점장에게 민감 계산값 없이 합계 불일치와 비민감 이상 후보를 보여준다", async ({
  ...
  await expect(metrics).toContainText("결제수단 합계와 총매출 차이");
  ...
  await expect(page.locator("main")).not.toContainText(
    /costOfGoodsSold|grossProfit|grossMarginRate|operatingProfit|productivity|inventoryAmount|differenceAmount|93\.0%|(^|[^0-9])8,000원/,
  );
});
