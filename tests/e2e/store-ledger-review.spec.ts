...
test("검토 화면은 지점장에게 민감 계산값 없이 합계 불일치와 비민감 이상 후보를 보여준다", async ({
  ...
  // 보완(2026-06-22 WO-01): 결제차액(총매출 - 결제 합계 - 지출 합계)은 본사 역할이므로
  ...
  // 판매계획가 입력 위치가 3단계 재고로 이동한 현재 안내를 검증한다.
  ...
  await expect(warningSection).not.toContainText("-10,000원");
  // 부족 방향은 정상 판매이므로 과거의 잘못된 조정 기록이 남아 있어도 재고 신호를 만들지 않는다.
  await expect(warningSection).not.toContainText("판매 추정 확인");
  await expect(warningSection).not.toContainText("재고 확인 필요");
...
test("7단계 추정 매출은 3단계 재고에서 입력한 판매계획가를 우선 반영한다", async ({
  page,
}) => {
  const suffix = randomUUID().slice(0, 8);
  const actorId = await getHeadquartersUserId();
  ...
  // 3단계 재고에서 저장된 판매가 계획(StoreSalesPricePlan).
  ...
  // 판매가 계획이 반영됐으므로 cost 폴백 표시와 3단계 재고 안내 링크가 없다.
