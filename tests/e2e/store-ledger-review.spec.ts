...
test("검토 화면은 지점장에게 민감 계산값 없이 합계 불일치와 비민감 이상 후보를 보여준다", async ({
  ...
  await expect(metrics).toContainText("비용");
  await expect(metrics).toContainText("매입");
  await expect(metrics).toContainText("재고");
  await expect(metrics).toContainText("손실");
  // WO(2026-06-25): work 단계 라벨은 입력 화면과 맞춘 "근무/인건비".
  await expect(metrics).toContainText("근무/인건비");
  // 근무인원/급여 항목/급여 합계는 계속 별도 metric으로 노출된다.
  await expect(metrics).toContainText("급여 항목");
  await expect(metrics).toContainText("급여 합계");
  ...
  // WO(2026-06-25): 안내 문구는 새 입력 위치(3단계 매입의 오늘 팔 가격(예상))와 맞춘다.
  await expect(topSoldSection).toContainText(
    "추정 매출은 3단계 매입의 오늘 팔 가격(예상)을 우선 사용합니다.",
  );
  await expect(topSoldSection).toContainText(product.name);
  await expect(topSoldSection).toContainText("7개");
  await expect(topSoldSection).toContainText("추정 매출 7,000원");
  // 판매가 계획이 없어 매입 단가로 폴백했으므로 "판매가 미반영"과 3단계 매입 안내 링크가 보인다.
  await expect(topSoldSection).toContainText("판매가 미반영");
  await expect(
    topSoldSection.getByRole("link", {
      name: "3단계 매입에서 오늘 팔 가격 입력",
    }),
  ).toHaveAttribute(
    "href",
  ...
  // 내부 필드 키와 차단 지표(매출원가/매출이익/영업이익/인당생산성)는 계속 노출 금지.
  // 마진율(%)과 총 재고금액(8,000원)은 위에서 노출을 확인했으므로 제외한다.
  await expect(page.locator("main")).not.toContainText(
    /costOfGoodsSold|grossProfit|grossMarginRate|operatingProfit|productivity|inventoryAmount|differenceAmount/,
  );
});

test("7단계 추정 매출은 3단계 매입에서 입력한 오늘 팔 가격을 우선 반영한다", async ({
  page,
}) => {
  const suffix = randomUUID().slice(0, 8);
  const actorId = await getHeadquartersUserId();
  ...
  // 3단계 매입에서 "오늘 팔 가격(예상)"으로 저장된 판매가 계획(StoreSalesPricePlan).
  ...
  const topSoldItem = topSoldSection.locator("li").filter({
    hasText: product.name,
  });
  await expect(topSoldItem).toContainText("7개");
  await expect(topSoldItem).toContainText("추정 매출 21,000원");
  // 판매가 계획이 반영됐으므로 품목 줄에 "판매가 미반영"이 붙지 않고 안내 링크도 없다.
  // (섹션 상단 안내 문구에는 설명용으로 "판매가 미반영"이 포함되므로 li 단위로 검증한다.)
  await expect(topSoldItem).not.toContainText("판매가 미반영");
  await expect(
    topSoldSection.getByRole("link", {
      name: "3단계 매입에서 오늘 팔 가격 입력",
    }),
...
test("검토 화면은 누락 항목 링크와 모바일 읽기 상태를 제공한다", async ({
  ...
  await expect(
    missingSection.getByRole("link", { name: "비용 단계로 이동" }),
