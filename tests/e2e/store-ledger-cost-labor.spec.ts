...
// WO-09: 지출 단계 화면에는 지출 항목 표시명(alias) 편집기가 함께 렌더링되고,
// 그 편집기에도 type="button" 저장 버튼이 있다. 지출 폼 제출 버튼은 type="submit"
...
test("지점장은 지출 항목을 여러 건 저장하고 재방문 시 유지한다", async ({
  ...
  await page.getByRole("textbox", { name: "지출 금액" }).nth(0).fill("3000");
  ...
  await page.getByRole("textbox", { name: "지출 금액" }).nth(1).fill("5000");

  await costSaveButton(page).click();
  await expect(
    page
      .getByRole("status")
      .filter({ hasText: "지출 항목 2건을 저장했습니다." }),
  ).toBeVisible();

  await page.reload();

  await expect(page.getByRole("textbox", { name: "지출 금액" })).toHaveCount(2);
  await expect(
    page.locator("section").filter({ hasText: "지출 합계" }),
  ).toContainText("8,000원");
  await expect(
    page.locator("section").filter({ hasText: "마지막 서버 저장 지출 합계" }),
  ).toContainText("8,000원");
});

test("지점장이 지출 항목 표시명(alias)을 바꾸면 선택지에 반영되고, 비우면 본사 등록명으로 되돌아간다", async ({
  page,
}) => {
  await login(page);
  ...
  await expect(
    page.getByRole("heading", { name: "지출 항목 표시명" }),
  ).toBeVisible();
...
test("비활성 지출 항목 코드는 기존 장부 표시만 유지되고 신규 선택지에는 없다", async ({
  page,
}) => {
  await login(page);
  ...
  // 기존에 남은 지출 행을 정리한 뒤 비활성 코드 행을 심는다.
  await prisma.ledgerExpense.deleteMany({
  ...
  const existingSelect = page.getByLabel("지출 항목", { exact: true }).nth(0);
  await expect(existingSelect).toHaveValue(inactiveCode.id);
  const existingOptions = await existingSelect
    .locator("option")
    .allTextContents();
  expect(existingOptions).toContain("지출 항목 선택");
...
test("지출 단계 미저장 변경 이동 dialog에서 저장 후 다음 단계로 이동한다", async ({
  page,
}) => {
  await login(page);
  ...
  await page.goto(`/app/store-entry?storeId=${STORY_STORE_ID}&step=cost`);
  await page.getByRole("button", { name: "항목 추가" }).click();
  await page.getByLabel("지출 항목", { exact: true }).selectOption(code.id);
  await page.getByRole("textbox", { name: "지출 금액" }).fill("9000");
...
test("지출 저장 실패 시 한국어 오류와 재시도 동작이 표시된다", async ({
  page,
}) => {
  await login(page);
  ...
  await page.getByRole("button", { name: "항목 추가" }).click();
  await page.getByLabel("지출 항목", { exact: true }).selectOption(code.id);
  await page.getByRole("textbox", { name: "지출 금액" }).fill("4400");
...
test("지점장은 근무인원과 특이사항을 저장하고 민감 회계 지표를 보지 않는다", async ({
  ...
  await page.getByRole("button", { name: "항목 추가" }).click();
  await page.getByLabel("지출 항목", { exact: true }).selectOption(code.id);
  await page.getByRole("textbox", { name: "지출 금액" }).fill("3000");
  await costSaveButton(page).click();
  await expect(
    page
      .getByRole("status")
  ...
  const summary = page.locator("section").filter({ hasText: "지출 합계" });

  await expect(summary).toContainText("지출 합계");
...
// WO-10(2026-06-28): 급여액은 본사 전용. 지점장은 근무자 명단/메모만 저장하고
// 급여 금액 입력란과 급여 합계 금액을 볼 수 없다. 금액은 본사가 별도로 관리한다.
test("지점장은 근무자 명단을 저장하되 급여 금액 입력란과 합계 금액은 보이지 않는다", async ({
  page,
}) => {
  await login(page);
...
test("근무 단계는 근무인원/이름 명칭과 근무자 입력을 유지하며 급여 UI를 숨긴다", async ({
  page,
}) => {
  await login(page);
...
test("지출 단계 검증 실패 시 첫 오류 필드로 포커스가 이동한다", async ({
  ...
  await expect(page.getByText("지출 항목을 선택해 주세요.")).toBeVisible();
  const expenseCodeInput = page.getByLabel("지출 항목", { exact: true });
...
test("390px에서 지출/근무 단계는 숫자 키패드 및 터치 타깃이 충족된다", async ({
  page,
}) => {
  ...
  const amountInput = page.getByRole("textbox", { name: "지출 금액" }).first();
