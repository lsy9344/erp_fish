...
test("지점장은 근무인원과 특이사항을 저장하고 영업이익·인당생산성을 본다", async ({
  page,
}) => {
  await login(page);
  ...
  const summary = page.locator("section").filter({ hasText: "영업이익" });

  await expect(summary).toContainText("영업이익");
  await expect(summary).toContainText("7,000원");
  ...
  await expect(page.getByText("저장됐습니다.")).toBeVisible();
  await expect(summary).toContainText("인당생산성");
  await expect(summary).toContainText("3,500원");
});

test("근무인원 0이면 인당생산성은 계산 불가로 표시된다", async ({ page }) => {
  await login(page);
  ...
  await expect(page.getByText("저장됐습니다.")).toBeVisible();
  const summary = page.locator("section").filter({ hasText: "인당생산성" });

  await expect(summary).toContainText("계산 불가");
});
