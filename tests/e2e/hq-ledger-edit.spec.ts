...
test("ClosePreflight 차단 항목은 표와 보완 링크를 보여주고 마감 확정을 막는다", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "차단 항목 보완 필요" }),
  ).toBeDisabled();

  const salesLink = salesRow.getByRole("link", {
    name: "기존 입력 단계에서 보완",
  });
  await expect(salesLink).toHaveAttribute(
    "href",
    /\/app\/store-entry\?storeId=store-story-4-4-preflight-blocked&date=\d{4}-\d{2}-\d{2}&step=sales/,
  );
  ...
  await expect(page.getByText("스토리4-4 보완필요점").first()).toBeVisible();
});
