...
test("본사는 일별 아침 회의 리포트에서 지점별 상태와 정정 반영 숫자를 본다", async ({
  ...
  await expect(closedRow).toContainText(/회의 반영 완료|지연 제출/);
...
test("본사는 좁은 화면에서도 일별 아침 회의 리포트 핵심 상태를 본다", async ({
  ...
  await expect(closedCard).toContainText("본사마감");
  await expect(closedCard).toContainText(/회의 반영 완료|지연 제출/);
...
test("본사는 일별 리포트에서 기간 비교로 이동해 선택 기간의 지점별 실적을 본다", async ({
  ...
  await expect(inProgressRow).toContainText("입력중 1일");
