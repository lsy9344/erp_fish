...
async function cleanupStory15Data() {
...
test("본사는 품목 마스터 목록과 검색, 구분, 상태 필터를 볼 수 있다", async ({
  ...
  await expect(page.getByRole("heading", { name: "품목 마스터" })).toBeVisible();
  ...
  await expect(page.getByRole("cell", { name: "스토리15 냉동고등어" })).toBeVisible();
  await expect(productRow(page, "스토리15 생물갈치")).toContainText("비활성");
  ...
  await expect(page.getByRole("cell", { name: "스토리15 냉동고등어" })).toBeVisible();
  await expect(page.getByRole("cell", { name: "스토리15 생물갈치" })).toHaveCount(0);
  ...
  await expect(page.getByRole("cell", { name: "스토리15 생물갈치" })).toBeVisible();
});
...
test("본사는 품목을 생성, 수정, 비활성 처리하고 감사 로그를 남긴다", async ({
  ...
  const productName = `스토리15 오징어 ${suffix}`;
  const editedName = `스토리15 손질오징어 ${suffix}`;
...
test("품목 수정은 보이지 않는 이전 활성 상태를 되살리지 않는다", async ({
  ...
  const suffix = Date.now().toString(36);
  const productName = `스토리15 상태경합 ${suffix}`;
  const productId = await seedProduct({ name: productName });

  await login(page, "hq@example.com");
  await page.goto("/app/master-data/products");

  await productRow(page, productName).getByRole("button", { name: "수정" }).click();
...
test("본사는 매입 기준을 생성, 수정, 비활성 처리하고 연결 품목 상태를 반영한다", async ({
  ...
  const suffix = Date.now().toString(36);
  const activeProduct = `스토리15 기준고등어 ${suffix}`;
  const inactiveProduct = `스토리15 비활성품목 ${suffix}`;
  ...
  await seedPurchaseStandard({
    ...
    referenceInfo: "story15 inactive product standard",
    isActive: false,
  });
  const activeStandardInactiveProduct = `스토리15 비활성품목활성기준 ${suffix}`;
  ...
  await seedPurchaseStandard({
    ...
    referenceInfo: "story15 inactive product active standard",
    isActive: true,
  });
  ...
  await page.getByLabel("참조 정보").fill(`story15 reference ${suffix}`);
  await page.getByRole("button", { name: "저장" }).click();
  ...
  await page.getByLabel("참조 정보").fill(`story15 edited ${suffix}`);
  await page.getByRole("button", { name: "저장" }).click();

  await expect(standardRow(page, activeProduct)).toContainText("9,900원");
  await expect(standardRow(page, activeProduct)).toContainText(`story15 edited ${suffix}`);
  ...
  await expect(page.getByRole("cell", { name: inactiveProduct })).toHaveCount(0);
  await expect(
...
test("매입 기준 수정은 보이지 않는 이전 활성 상태를 되살리지 않는다", async ({
  ...
  const suffix = Date.now().toString(36);
  const productName = `스토리15 기준상태경합 ${suffix}`;
  const productId = await seedProduct({ name: productName });
  const standardId = await seedPurchaseStandard({
    productId,
    standardUnitPrice: 8000,
    referenceInfo: `story15 stale ${suffix}`,
    isActive: true,
  });
  ...
  await standardRow(page, productName)
    .getByRole("button", { name: "수정" })
    .click();
  await page.getByLabel("참조 정보").fill(`story15 stale edited ${suffix}`);
  ...
  await expect
    ...
    .toBe(`story15 stale edited ${suffix}`);
  await expect
    .poll(async () => {
      const rows = await prisma.$queryRawUnsafe<Array<{ isActive: boolean }>>(
        `SELECT "isActive" FROM "PurchaseStandard" WHERE id = $1`,
...
test("품목과 매입 기준 폼은 한국어 검증 오류와 첫 오류 포커스를 제공한다", async ({
  ...
  const productName = `스토리15 검증 ${Date.now().toString(36)}`;
  ...
  await expect(page.getByLabel("품목명")).toHaveAttribute("aria-invalid", "true");
...
test("지점장은 품목과 매입 기준 관리 URL에서 데이터를 볼 수 없다", async ({
  ...
  await seedProduct({ name: "스토리15 지점장차단" });

  await login(page, "manager@example.com");

  await page.goto("/app/master-data/products");
  await expect(page).toHaveURL(/\/app\/unauthorized/);
  await expect(page.getByRole("heading", { name: "품목 마스터" })).toHaveCount(0);
  await expect(page.getByText("스토리15 지점장차단")).toHaveCount(0);
