...
const STORY52_LEDGER_DATE = getTodayKstDateParam();
...
async function openProductCreateDialog(page: Page) {
  await clickAndExpectVisible(
    page.getByRole("button", { name: "품목 추가" }),
    page.getByLabel("품목명"),
  );
}

async function openProductEditDialog(page: Page, productName: string) {
  await clickAndExpectVisible(
    productRow(page, productName).getByRole("button", { name: "수정" }),
    page.getByLabel("참고 단가(선택)"),
  );
}

async function openPurchaseStandardCreateDialog(page: Page) {
  await clickAndExpectVisible(
    page.getByRole("button", { name: "참고 단가 추가" }),
    page.getByLabel("품목", { exact: true }),
  );
}

async function openPurchaseStandardEditDialog(page: Page, productName: string) {
  await clickAndExpectVisible(
    standardRow(page, productName).getByRole("button", { name: "수정" }),
...
function cacheBustPath(path: string) {
  return `${path}?e2e=${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
}

async function gotoProductsPage(page: Page) {
  await page.goto(cacheBustPath("/app/master-data/products"));
}

async function gotoPurchaseStandardsPage(page: Page) {
  await page.goto(cacheBustPath("/app/master-data/purchase-standards"));
}
...
test("본사는 품목 마스터 목록과 검색, 구분, 상태 필터를 볼 수 있다", async ({
  ...
  await expect(
    page.getByRole("columnheader", { name: "참고 단가(선택)" }),
  ).toBeVisible();
...
test("본사는 품목을 생성, 수정, 비활성 처리하고 감사 로그를 남긴다", async ({
  ...
  await openProductCreateDialog(page);
  ...
  await page.getByLabel("참고 단가(선택)").fill("18000");
  ...
  await openProductEditDialog(page, productName);
  await page.getByLabel("품목명").fill(editedName);
  await page.getByLabel("참고 단가(선택)").fill("21000");
  await page.getByRole("button", { name: "저장" }).click();
...
test("품목 수정은 보이지 않는 이전 활성 상태를 되살리지 않는다", async ({
  ...
  await openProductEditDialog(page, productName);
  await page.getByLabel("참고 단가(선택)").fill("13000");
...
test("본사는 품목 참고 단가를 생성, 수정, 비활성 처리하고 연결 품목 상태를 반영한다", async ({
  page,
}) => {
  const suffix = Date.now().toString(36);
  ...
  await login(page, "hq@example.com");
  await gotoPurchaseStandardsPage(page);
  ...
  await openPurchaseStandardCreateDialog(page);
  await expect(
  ...
  await openPurchaseStandardEditDialog(page, activeProduct);
...
test("품목 참고 단가 수정은 보이지 않는 이전 활성 상태를 되살리지 않는다", async ({
  page,
}) => {
  const suffix = Date.now().toString(36);
  ...
  await openPurchaseStandardEditDialog(page, productName);
...
test("품목과 참고 단가 폼은 한국어 검증 오류와 첫 오류 포커스를 제공한다", async ({
  page,
}) => {
  ...
  await page.getByLabel("참고 단가(선택)").fill("-1");
  await page.getByRole("button", { name: "저장" }).click();
  ...
  await page.getByRole("textbox", { name: "참고 단가" }).fill("");
  await page.getByLabel("참조 정보").fill(" ");
  await page.getByRole("button", { name: "저장" }).click();

  await expect(
    page.getByText("참고 단가 또는 참조 정보를 입력해 주세요."),
  ).toBeVisible();
...
test("중복 품목 생성은 한국어 field error로 막고 첫 오류에 포커스한다", async ({
  ...
  await page.getByLabel("참고 단가(선택)").fill("12000");
  await page.getByRole("button", { name: "저장" }).click();

  await expect(page.locator("#product-name-error")).toHaveText(
  ...
  await expect(
    prisma.product.count({
      where: { name: productName, category: "생물", spec: "3kg" },
    }),
  ).resolves.toBe(1);
});
...
test("품목 변경과 비활성화는 신규 장부 선택지에서만 제외하고 저장된 매입 snapshot은 유지한다", async ({
  ...
  await expect(
    page.getByText(`상세: ${productName} · 생물 · 1kg · 참고 단가 16,000원`, {
      exact: true,
    }),
  ).toBeVisible();
...
test("지점장은 품목과 품목 참고 단가 관리 URL에서 데이터를 볼 수 없다", async ({
  page,
}) => {
