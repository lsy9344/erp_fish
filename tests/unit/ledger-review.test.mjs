...
test("store manager review exposes estimated top sold items derived from inventory flow (WO-04)", () => {
  ...
  // 판매수량은 재고 흐름(전일+매입+전환입고-전환출고-손실-당일재고)으로만 추정한다.
  assert.match(querySource, /buildStoreManagerTopSoldItems/);
  assert.match(
    querySource,
    /item\.previousQuantity\s*\+\s*item\.purchasedQuantity\s*-\s*\(item\.conversionOutQuantity\s*\?\?\s*0\)\s*\+\s*\(item\.conversionInQuantity\s*\?\?\s*0\)\s*-\s*item\.lossQuantity\s*-\s*item\.currentQuantity/,
  );
