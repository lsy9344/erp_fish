...
//
// 실행: node --experimental-strip-types scripts/reset-and-seed-prevday.mjs --yes
import "./_loadenv.mjs";
...
requireResettableDatabaseUrl(process.env.DATABASE_URL, process.env);
...
async function getActors() {
  ...
  if (!hq || !sm)
    throw new Error(
      "HQ/STORE_MANAGER 사용자가 없습니다. prisma seed를 먼저 실행하세요.",
    );
...
async function loadAliasMaps() {
  const [storeAliases, productAliases] = await Promise.all([
    db.storeExternalAlias.findMany({
      where: { provider: ECOUNT_PROVIDER },
      select: { rawName: true, storeId: true },
    }),
    db.productExternalAlias.findMany({
      where: { provider: ECOUNT_PROVIDER },
      select: { rawName: true, rawSpec: true, productId: true },
    }),
  ]);
  const storeByRaw = new Map();
  for (const a of storeAliases)
    storeByRaw.set(storeAliasKey(a.rawName), a.storeId);
  const productByRaw = new Map();
  for (const a of productAliases)
    productByRaw.set(productAliasKey(a.rawName, a.rawSpec), a.productId);
...
async function importAndCommitEcount(hqId, parsed, aliases) {
  ...
  const batch = await db.ecountImportBatch.create({
    data: {
      ...
      businessDate: utcDate(ECOUNT_DAY),
      status: "PREVIEW",
      uploadedById: hqId,
    },
  });
  ...
  if (batchStatus !== "READY") {
    ...
    throw new Error(
      `이카운트 batch가 READY가 아님(${batchStatus}). 매핑 누락: ${JSON.stringify(breakdown)}`,
    );
  }
  ...
  const committed = await db.$transaction(
    async (tx) => {
          ...
          const existing = await tx.dailyLedger.findUnique({
            where: {
              storeId_closingDate: {
                ...
                closingDate: utcDate(ECOUNT_DAY),
              },
            },
            select: { id: true },
          });
      ...
      for (const ledgerId of affected)
  ...
  for (let di = 0; di < RECENT_DAYS.length; di++) {
    ...
    for (let pi = 0; pi < products.length; pi++) {
      ...
      if (seeded(salt * 1000 + di * 100 + pi) < 0.5) {
        const qty =
          1 + Math.floor(seeded(salt * 1000 + di * 100 + pi + 7) * 10); // 1~10
    ...
    for (let pi = 0; pi < products.length; pi++) {
      ...
      if (seeded(salt * 2000 + di * 100 + pi + 3) < 0.18) {
        ...
        const code =
          lossCodes[
            Math.floor(seeded(salt + di + pi) * lossCodes.length) %
              lossCodes.length
          ];
        await db.ledgerLossItem.create({
          data: {
            dailyLedgerId: ledger.id,
            productId: p.id,
            ledgerInputCodeId: code.id,
            productName: p.name,
            productCategory: p.category,
            productSpec: p.spec,
            unitPrice: p.unitPrice,
            lossTypeName: code.name,
            quantity: qty,
            recoveredAmount: 0,
            amount: p.unitPrice * qty,
            reason: "테스트 손실",
    ...
    for (let pi = 0; pi < products.length; pi++) {
      ...
      const sold = Math.max(
        0,
        Math.min(available, Math.round(available * sellRatio)),
      );
      const currentQuantity = available - sold;
      estSales += sold * p.unitPrice;

      const carryoverSource =
        di === 0 ? "OPENING_SNAPSHOT" : "PREVIOUS_CLOSED_LEDGER";
      const carryoverStatus =
        di === 0 ? "OPENING_CARRYOVER" : "PREVIOUS_CARRYOVER";
    ...
    await db.dailyLedger.update({
      ...
      data: {
        ...
        cashAmount: cash,
        cardAmount: card,
        otherPaymentAmount: other,
        workerCount: 2,
        authorDisplayName: "지점장",
        submittedById: !isToday ? smId : null,
        submittedAt: !isToday ? utcDate(day) : null,
        closedById: !isToday ? hqId : null,
        closedAt: !isToday ? utcDate(day) : null,
        updatedById: smId,
      },
    });
    ...
    console.log(
      `   [${store.name}] ${day} status=${status} 매입=${purchasedQty.size} 손실=${lossQty.size}`,
    );
  }
}

async function main() {
  const target = describeDatabaseTarget(process.env.DATABASE_URL, process.env);
  console.log(
  ...
  for (const line of parsed.lines) {
    ...
    const productId = aliases.productByRaw.get(
      productAliasKey(line.rawProductName, line.productSpec),
    );
  ...
  const stores = await db.store.findMany({
    where: { id: { in: [...storeProducts.keys()] } },
