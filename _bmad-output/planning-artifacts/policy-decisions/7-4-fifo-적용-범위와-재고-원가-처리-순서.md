...
| 결정 항목 | 결정 |
| --- | --- |
| OQ-7 적용 범위 | FIFO 계산 대상은 모든 재고품목이다. 정책상 예외 품목을 두지 않는다. 품목 매핑/단가/수량/입고 근거가 불완전한 행은 FIFO 대상에서 제외하지 않고, 확정 계산 대신 상태값(`기준 확인 필요`, `데이터 부족`, `검토 필요`, `재검증 필요`, `계산 불가`)으로 반환한다. |
| 근거 부족 row 처리 | mapping 실패, 검토 필요, 기준 단가/매입 lot 누락, 단위 불명확, 음수 lot 발생, 승인자 없음 상태는 "FIFO 적용 제외"가 아니라 "확정 계산 불가 상태"로 분리한다. |
| 승인 전 표시 | 승인 전에는 기존 MVP 기본 계산값과 `기준 확인 필요`를 분리해 표시한다. FIFO 확정 원가나 FIFO 재고금액으로 이름을 바꾸지 않는다. |
| OQ-17 처리 순서 | 전일 확정 이월 lot -> 당일 본사 출고/지점 입고 lot -> 승인된 조정 증가 lot -> 반품/void 입고 취소 -> 판매 수량 FIFO 차감 -> 손실/폐기/떨이 FIFO 차감 -> 승인된 조정 감소 -> 마감 snapshot 순서로 처리한다. 판매 수량은 지점 마감 입력값으로 역산한다. |
| 판매 예정가 vs FIFO 원가 | 판매 예정가는 매출 후보 계산(판매 수량 x 판매 예정가)에만 쓰고 FIFO 원가 계산에는 쓰지 않는다. FIFO 원가는 본사 출고/입고 단가와 이월 lot 단가로만 계산한다. |
...
FIFO 계산 대상은 모든 재고품목이다. 정책상 예외 품목을 두지 않는다. 운영 방식은 매일 아침 본사가 이카운트 엑셀처럼 지점에 출고하고, 지점장이 그 물량을 현장 판매 예정가로 판매한 뒤 영업 종료 후 장부에 현재 재고와 판매 예정가를 입력하는 흐름이다. 따라서 특정 품목만 고르지 않고 모든 재고품목을 FIFO 계산 후보로 삼는다.

다만 품목 매핑/단가/수량/입고 근거가 빠진 행은 FIFO 대상에서 제외하는 것이 아니라, 확정 계산 대신 상태값으로 반환한다. 아래 표의 상태는 "적용 제외"가 아니라 "확정 계산 불가 상태"를 의미한다.

| 조건 | FIFO 계산 후보 | 계산 결과 상태 | 비고 |
| --- | --- | --- | --- |
| 정규 품목 `name/category/spec` 승인됨 | 대상 | 확정 계산 가능 | Story 7.3 승인과 mapping version 필요 |
| 매입 lot source, 수량, 단가, 매입일자가 모두 있음 | 대상 | 확정 계산 가능 | `PurchaseLot` 또는 동등한 lot record 필요 |
| 단가 다른 매입이 여러 개 있고 lot 잔량 추적 가능 | 대상 | 확정 계산 가능 | CAP-7 핵심 대상 |
| mapping 상태가 `mapping_failed` | 대상 | `기준 확인 필요` | 정규 품목이 없어 lot 귀속 불가. 확정 금액 대신 상태 반환 |
| mapping 상태가 `needs_review` | 대상 | `검토 필요` 또는 `기준 확인 필요` | 본사 검수 승인 전 확정 금지 |
| mapping 상태가 `basis_missing` | 대상 | `데이터 부족` | 단가, 수량, 규격, 매입일자 중 필수 근거 결측 |
| mapping 상태가 `pending_review` | 대상 | `검토 필요` | 승인 대기 상태 |
| preview 이후 mapping/version 변경으로 `revalidation_required` | 대상 | `재검증 필요` | reprocess 후 commit 필요 |
| 수량/단위가 음수, 0, 혼합 단위 등으로 해석 불가 | 대상 | `계산 불가` 또는 `기준 확인 필요` | row-level 오류와 감사 로그 필요 |

### 근거 부족 행의 처리 기준

모든 재고품목이 FIFO 계산 대상이며, 근거가 불완전한 행은 확정 계산 대신 상태값으로 반환한다. 행 단위로 아래처럼 분류한다.

| 대상 구분 | 확정 계산 가능 조건 | 상태값으로 반환하는 경우 | 후속 재검토 조건 |
| --- | --- | --- | --- |
| 일반 매입 수산물 | 정규 품목 승인, 매입 lot 추적 가능, 단가/수량/규격 확정 | 원문 품목만 있고 정규 mapping 없음 → `기준 확인 필요` | Story 7.3 승인과 mapping migration 완료 |
| 단가 변동이 잦은 품목 | 같은 품목/규격에 복수 단가 lot 존재 | lot 잔량이 음수거나 매입일자 불명확 → `계산 불가` | lot replay가 음수 없이 통과 |
| 업로드 매입 품목 | commit 완료 import row, source row id, mapping version, 단가 확정 | preview-only, pending review, revalidation required → `검토 필요`/`재검증 필요` | 재처리 후 본사 승인 |
| 수동 매입 품목 | 지점 또는 본사 수동 입력 row, 감사 이벤트 존재 | 단가 또는 수량 결측, 원문 혼합 품목 → `데이터 부족` | 본사 수동 수정 승인과 lot 생성 근거 확보 |
| 손실/폐기/떨이 대상 품목 | 손실 유형과 수량이 확정되고 lot 차감 가능 | 손실 유형 불명확, 수량 초과 → `계산 불가` | 손실 코드 정책과 승인된 차감 순서 확보 |
...
운영 흐름은 "본사 아침 출고 -> 지점 현장 판매 -> 마감 재고 입력"이다. 아침 본사 이카운트 출고분은 당일 판매 전 지점 입고 lot으로 본다. 판매 수량은 지점장이 매번 판매 행을 입력하는 방식이 아니라 마감 입력값으로 역산한다.

| 순서 | 이벤트 | lot 잔량 영향 | 원가/재고금액 영향 | 매출원가/손실 영향 | 상태/차단 |
| --- | --- | --- | --- | --- | --- |
| 0 | 전일 확정 이월 lot 로드 | 전날 마감 후 남은 lot 잔량을 오늘 시작 재고로 로드 | 시작 재고금액 산출 | 없음 | 마감 전 후보는 `기준 확인 필요` |
| 1 | 당일 본사 출고/지점 입고 lot 생성 | 아침 이카운트 출고분을 당일 판매 전 입고 lot으로 증가 | 출고/입고 단가 기준 lot 원가 증가 | 없음 | mapping/단가 미확정 row는 차단 |
| 2 | 승인된 조정 증가 lot | 본사 승인 또는 명확한 근거가 있는 증가분만 새 lot으로 append | 승인 단가로 재고금액 증가 | 없음 | 단가 기준 없으면 `기준 확인 필요` |
| 3 | 반품/void 입고 취소 | 당일 입고 자체가 취소된 경우 판매 차감 전에 lot에서 제거 | 원 lot 원가를 역분개 | 매출원가가 아니라 매입 차감 | 대상 lot 불명확 시 `기준 확인 필요` |
| 4 | 판매 수량 FIFO 차감 | 가장 오래된 잔량부터 차감 | 남은 lot 기준 재고금액 재계산 | 차감 lot 원가 합계가 판매원가 후보 | 판매 수량 역산 불가 시 `기준 확인 필요` |
| 5 | 손실/폐기/떨이 FIFO 차감 | 판매 후 남은 오래된 lot부터 비정상 차감 | 재고금액 감소 | 손실/폐기/떨이 금액은 차감 lot 원가로 산출 후보 | 손실 유형/수량 초과 시 차단 |
| 6 | 승인된 조정 감소 | 본사 승인 조정으로 FIFO 순서 또는 지정 lot 차감 | 조정 전/후 금액과 차이 기록 | 매출원가가 아니라 조정 차이 | 음수 lot이면 저장 차단 |
| 7 | 마감 snapshot 생성 | 남은 lot 잔량 snapshot 생성 | 확정 이월 금액 저장 | 리포트 기준점 확정 | 정정 발생 시 재확인 필요 |

판매 수량은 지점 마감 입력값으로 역산한다.
...
- 판매 예정가: 지점장이 칠판이나 장부에 적는 판매 가격이다. 매출 후보(판매 수량 x 판매 예정가) 계산에만 쓴다.
- FIFO 원가: 본사 출고/입고 단가나 이월 lot 단가다. 판매원가 후보, 재고금액 후보 계산에 쓴다.
- 판매 예정가를 바꿔도 FIFO 원가와 재고금액 후보는 바뀌지 않는다.

판매 수량 역산은 현 MVP의 `previousQuantity + purchasedQuantity - lossQuantity` 기본 계산과 다르다. 후속 FIFO 구현 story에서 판매 수량 역산식과 매출원가 후보 표시 범위가 테스트 가능한 기준으로 확정되지 않으면 FIFO 매출원가를 `ok`로 확정하지 않는다.
...
- 적용 범위: 모든 재고품목 적용으로 결정했고, 근거 부족 행은 적용 제외가 아니라 상태값(`기준 확인 필요`/`데이터 부족`/`검토 필요`/`재검증 필요`/`계산 불가`)으로 반환하도록 표로 정의했다.
