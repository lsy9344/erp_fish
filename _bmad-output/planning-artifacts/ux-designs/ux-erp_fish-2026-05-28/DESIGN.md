...
브랜드 델타는 primary blue, sky accent, warning amber, success green, 숫자 데이터용 `tabular-nums`, 더 각진 radius, 장부 상태 배지, 이상 신호 칩, 추가 상태 배지, 그리고 공통 MetricCard 표면이다. 기능 동작보다 시각 언어를 시스템화하는 문서다.
...
- **Additional Status Badge (추가 상태 배지)** — `확인 필요`, `매핑 실패`, `업로드 반영`, `읽기 전용`, `발송 실패`. pill 형태와 텍스트 레이블을 유지하고, 실패/위험은 아이콘 또는 설명 텍스트와 함께 쓴다.
