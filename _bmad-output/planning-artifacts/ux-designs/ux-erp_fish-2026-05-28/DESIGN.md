...
colors:
  ...
  success-dark: "#4ADE80"
  success-foreground-dark: "#0A0F1E"
typography:
...
components:
  ...
  badge-holiday:
    ...
    radius: "{rounded.full}"
    style: "italic"
  badge-confirm-needed:
    label: "확인 필요"
    background: "{colors.warning} / 0.12"
    foreground: "{colors.warning}"
    border: "1px solid {colors.warning} / 0.24"
    radius: "{rounded.full}"
  badge-review-needed:
    label: "검토 필요"
    background: "{colors.warning} / 0.12"
  ...
  badge-manual-price:
    label: "수동 단가 포함"
    background: "{colors.warning} / 0.1"
    foreground: "{colors.warning}"
    border: "1px solid {colors.warning} / 0.2"
    radius: "{rounded.full}"
  badge-mapping-failed:
    label: "매핑 실패"
    background: "destructive / 0.1"
    foreground: "destructive"
    border: "1px solid destructive / 0.24"
    radius: "{rounded.full}"
  badge-import-applied:
    label: "업로드 반영"
    background: "{colors.primary} / 0.08"
    foreground: "{colors.primary}"
    border: "1px solid {colors.primary} / 0.2"
    radius: "{rounded.full}"
  ...
  inventory-row-readonly:
    background: "muted / 0.45"
    border-left: "2px solid border"
    cursor: "default"
  import-row-failed:
    background: "destructive / 0.04"
    border-left: "2px solid destructive"
  import-row-ready:
    background: "{colors.primary} / 0.04"
    border-left: "2px solid {colors.primary}"
  preflight-row-blocked:
    background: "destructive / 0.04"
    border-left: "2px solid destructive"
  preflight-row-warning:
    background: "{colors.warning} / 0.06"
...
브랜드 델타는 primary blue, sky accent, warning amber, success green, 숫자 데이터용 `tabular-nums`, 더 각진 radius, 장부 상태 배지, 이상 신호 칩, 검증/업로드/재확인 상태 배지, 그리고 공통 MetricCard 표면이다. 기능 동작보다 시각 언어를 시스템화하는 문서다.
...
- **리사이징 테이블 핸들:** 6px 폭 투명 hit area. hover/focus에서만 primary 색을 보여준다.
...
- **Additional Status Badge (추가 상태 배지)** — `확인 필요`, `검토 필요`, `이월 공백`, `데이터 부족`, `계산 불가`, `재확인 필요`, `매핑 실패`, `업로드 반영`, `읽기 전용`, `발송 실패`. pill 형태와 텍스트 레이블을 유지하고, 실패/위험은 아이콘 또는 설명 텍스트와 함께 쓴다.
- **Import Row State (업로드 행 상태)** — 검증 실패 행은 destructive 보더, 반영 준비 행은 primary 보더를 사용한다. 행별 실패 사유 텍스트를 반드시 함께 표시한다.
- **Upload Batch Badge (업로드 배치 배지)** — `업로드 미리검토`, `업로드 반영`, `업로드 무효`, `재처리`, `매핑 실패`를 구분한다. preview는 아직 장부 숫자에 반영되지 않았음을 설명 텍스트와 함께 표시한다.
- **Preflight Row State (마감 사전점검 행 상태)** — 차단 조건은 destructive 보더, 사유 입력으로 예외 가능한 조건은 warning 보더를 사용한다. 행에는 조건명, 영향 장부, 필요한 조치가 함께 보인다.
- **Conflict Row State (저장 충돌 행 상태)** — 내 입력값과 서버 최신값을 나란히 비교하는 행에 warning 보더를 사용한다. 마지막 수정자와 시각을 숫자/값과 같은 시각 계층에 둔다.
- **Readonly Inventory Row (읽기 전용 매입/재고 행)** — 본사 업로드 매입처럼 지점장이 수정할 수 없는 행은 muted 배경과 `읽기 전용` 배지로 표시한다. disabled 스타일만으로 이유를 숨기지 않는다.
- **Resizable Table Handle (테이블 리사이징 핸들)** — 관제판/전체 재고/상품 분석 대형 표의 컬럼 경계에 사용한다. hover/focus 상태를 제공하고 텍스트나 숫자를 덮지 않는다.
...
| 관제판은 `max-w-[1600px]`, 폼은 `max-w-2xl`로 폭을 분리한다                          | 모든 화면에 동일한 최대 폭을 적용한다                              |
...
| `확인 필요`와 `매핑 실패`는 배지와 설명 텍스트를 함께 표시한다                    | 계산 불가나 매핑 실패를 빈 값, 0원, 숨김 처리로 넘긴다             |
| `기준 확인 필요`, `데이터 부족`, `계산 불가`는 서로 다른 상태로 표시한다             | 정책 미확정, 분모 없음, 계산 오류를 같은 회색 빈칸으로 합친다       |
| 업로드 preview/commit/void/reprocess는 배치 배지와 감사 가능한 행 상태로 구분한다    | 업로드 상태를 파일명 하나나 성공/실패 색상만으로 표현한다           |
| 본사 업로드 매입은 `업로드 반영`과 `읽기 전용` 상태를 명확히 표시한다             | 지점장이 수정 가능한 수기 매입 행과 같은 스타일로 보여준다         |
| 리사이징 핸들은 데이터 셀을 가리지 않게 좁고 명확한 focus 상태를 둔다             | 컬럼 리사이징 때문에 숫자나 상태 배지가 겹치게 둔다                |
