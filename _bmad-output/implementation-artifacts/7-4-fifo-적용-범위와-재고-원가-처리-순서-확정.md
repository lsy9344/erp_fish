...
- [x] FIFO 적용 범위와 예외 기준을 결정 가능한 형태로 닫는다. (AC: 1, 5)
  ...
  - [x] 모든 재고품목을 FIFO 계산 대상으로 두고, 근거가 불완전한 행은 적용 제외가 아니라 상태값(`기준 확인 필요`/`데이터 부족`/`검토 필요`/`재검증 필요`/`계산 불가`)으로 반환하는 기준을 표로 정의한다. (2026-06-25 OQ-7 결정 반영)
...
- 2026-06-25: OQ-7/OQ-17 사용자 결정 반영. OQ-7을 "승인된 정규 품목 중 lot 근거가 완전한 품목에만 적용"(일부 적용)에서 "모든 재고품목이 FIFO 계산 대상이며, 근거가 불완전한 행은 적용 제외가 아니라 상태값으로 반환"으로 교정했다. OQ-17 처리 순서를 운영 흐름(아침 본사 출고=당일 판매 전 입고, 판매 수량=마감 입력값 역산) 기준으로 다시 정렬하고, 판매 예정가와 FIFO 원가의 역할 분리, 판매 수량 역산식을 명시했다. 정책 문서 갱신만 수행했고 `policy-gates.ts`의 FIFO gate는 해제하지 않았다(여전히 `policy-unconfirmed`). 위 2026-06-12 항목은 당시 기록으로 보존한다.
...
- Story 7.4 정책 산출물 작성 완료: FIFO는 전 품목 자동 적용이 아니라 승인된 정규 품목과 완전한 lot 근거가 있는 품목에 조건부 적용하는 것으로 정리했다. (※ 2026-06-25 OQ-7 결정으로 교정됨: 모든 재고품목이 FIFO 계산 대상이며 근거 부족 행은 적용 제외가 아니라 상태값으로 반환한다. Change Log 2026-06-25 항목 참조.)
